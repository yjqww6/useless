...
(define-syntax (surrogate stx)
  ...
  (define-splicing-syntax-class opt
    (pattern (~seq (~alt (~optional (~and #:use-wrapper-proc (~bind [wrapper? #t]))
                                    #:defaults ([wrapper? #'#f]))
                         (~optional (~seq #:set-surrogate setter:id)
                                    #:defaults ([setter #'set-surrogate]))
                         (~optional (~seq #:get-surrogate getter:id)
                                    #:defaults ([getter #'get-surrogate])))
                   ...)))
                ...
                #,(if use-surrogate-wrapper-proc?
                      #'(begin
                          ...
                          (define/public-final (get-surrogate-wrapper-proc)
                            surrogate-wrapper-proc))
                      #'(begin))
                (define/public-final (msw.setter new-surrogate)
                  (do-set-surrogate (λ (s) (set! the-surrogate s))
                                    ...
                                    '(ids ...)))
                (define/public-final (msw.getter) the-surrogate)
